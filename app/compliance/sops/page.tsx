...
export default function SOPsPage() {
  ...
  async function createSOP() {
    ...
    if (!error && data) {
      router.push(`/compliance/sops/${data.id}`);
    } else if (error) {
    ...
    <>
      <main className="flex-1 px-4 py-6 sm:px-6 lg:px-8 max-w-6xl w-full mx-auto space-y-6">
          <div className="flex items-center justify-between gap-2 flex-wrap">
            ...
            {isAdmin && (
              <button onClick={() => setShowNew(true)} className="btn-primary text-sm">+ New SOP</button>
            )}
          </div>
          {/* Filters */}
                ...
                <div key={sop.id} className="card p-4 hover:border-brand/40 transition-colors flex flex-col gap-3">
                  ...
                  <div className="flex items-center justify-between mt-auto pt-1">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {isAdmin && (
                        <Link href={`/compliance/sops/${sop.id}`} className="btn-secondary text-xs">Edit</Link>
                      )}
                    </div>
                  </div>
                </div>
